...
import { addTreatment, editTreatment } from '@/services/treatments.service';
...
export const AddOrEditTreatmentForm = ({
  isEdit,
  data,
  handleOpen,
}: {
  ...
  const onInvalidSubmit = (errors: FieldErrors<Treatment>) => {
    if (errors.nombre)
      toast.error(errors.nombre.message, { position: 'top-center' });
  };
  const onSubmit = async (values: Treatment) => {
    try {
      ...
      if (!isEdit) await addTreatment(PAYLOAD, access_token);
      else await editTreatment(PAYLOAD, access_token);
