...
import { editApplicationConsume } from '@/services/sprays.service';
...
export const EditProductConsumeForm = ({
  defaultValues,
  handleOpen,
}: Props) => {
  ...
  } = useForm<ApplicationWithConsume>({ defaultValues });

  const [isSubmitSuccessful, setIsSubmitSuccessful] = useState<
    boolean | undefined
  >(undefined);

  const onInvalidSubmit = (errors: FieldErrors<ApplicationWithConsume>) => {
    if (errors?.dosis)
  ...
  const onSubmit = async (values: ApplicationWithConsume) => {
    try {
      const PAYLOAD: ApplicationWithConsume = {
      ...
      await editApplicationConsume(PAYLOAD, access_token);
  ...
  return (
    ...
    >
      <Badge className='col-span-full w-fit' variant={'secondary'}>
        ...
        {SHORT_UNITY[defaultValues.producto?.unidad as UNITY]}
        /ha
      </Badge>
      <Input
        placeholder='Dosis'
