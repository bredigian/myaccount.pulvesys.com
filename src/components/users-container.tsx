...
import { getUsersByEnterpriseID } from '@/services/users.service';

interface Props {
  filter: string;
}

export default async function UsersContainer({ filter }: Props) {
  const access_token = (await cookies()).get('access_token');
  const refresh_token = (await cookies()).get('refresh_token');
  if (!access_token || !refresh_token) redirect('/', RedirectType.replace);

  const data = await getUsersByEnterpriseID(access_token.value, refresh_token);
      ...
      <TableBody>
        {filteredData.length === 0 ? (
          ...
          filteredData.map((usuario) => (
            <UserItem key={usuario.id} data={usuario} />
          ))
        )}
      </TableBody>
    </Table>
