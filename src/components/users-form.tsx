...
import { addUser, editUser } from '@/services/users.service';
...
export const AddOrEditUserForm = ({
  isEdit,
  data,
  handleOpen,
}: {
  ...
  const onInvalidSubmit = async (errors: FieldErrors<User>) => {
    if (errors?.nombre)
  ...
  const onSubmit = async (values: User) => {
    try {
      ...
      if (!isEdit) {
        ...
        await addUser(PAYLOAD, access_token);
      } else {
        const UPDATE_PAYLOAD: UpdateUserProps = {
          id: data?.id,
        ...
        await editUser(UPDATE_PAYLOAD, access_token);
      }
