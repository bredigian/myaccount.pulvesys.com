...
import { addCrop, editCrop } from '@/services/crops.service';
...
export const AddOrEditCropForm = ({
  isEdit,
  data,
  handleOpen,
}: {
  ...
  const onInvalidSubmit = (errors: FieldErrors<Crop>) => {
    if (errors.nombre)
      toast.error(errors.nombre.message, { position: 'top-center' });
  };
  const onSubmit = async (values: Crop) => {
    try {
      const PAYLOAD: Crop = {
        ...values,
        id: data?.id,
      ...
      if (!isEdit) await addCrop(PAYLOAD, access_token);
      else await editCrop(PAYLOAD, access_token);
