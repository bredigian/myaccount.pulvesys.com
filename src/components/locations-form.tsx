...
import { addLocation, editLocation } from '@/services/locations.service';
...
export const AddOrEditLocationForm = ({
  isEdit,
  data,
  handleOpen,
  storedData,
  ...
  const onInvalidSubmit = (errors: FieldErrors<Location>) => {
    if (errors.nombre)
  ...
  const onSubmit = async (values: Location) => {
    try {
      ...
      const lotes: Lot[] = polygons.map((i) => {
        return {
      ...
      if (!isEdit) await addLocation(PAYLOAD, access_token);
      else await editLocation(PAYLOAD, access_token);
