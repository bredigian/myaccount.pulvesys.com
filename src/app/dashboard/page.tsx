...
import SpraysContainer from '@/components/sprays-container';
import { SpraysMasonrySkeleton } from '@/components/masonry-skeleton';
...
export default async function SpraysPage({ searchParams }: Props) {
  ...
  return (
    <main className='space-y-4 p-4 pt-0'>
      ...
      <Suspense fallback={<SpraysMasonrySkeleton />} key={filter}>
        <SpraysContainer query={filter} />
      </Suspense>
    </main>
  );
}
